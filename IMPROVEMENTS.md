...
- **Model name is hardcoded** in `NewOpenAIModel` (`internal/llm/openai.go`, `model: "gpt-5.4-mini"`). When the model is deprecated the tool breaks and requires a code change + version bump (this already happened once — see commit `0acb529`). Add a `--model` flag / `WithModel` option so users can override without a release.
